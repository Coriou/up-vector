...
describe("SDK: query", () => {
	...
	beforeAll(async () => {
		...
		await index.upsert([
			{ id: "q1", vector: baseVec, metadata: { type: "animal", diet: "carnivore", pop: 900 } },
			{
				id: "q2",
				vector: randomVector(DIM),
				metadata: { type: "animal", diet: "herbivore", pop: 500 },
			},
			{ id: "q3", vector: randomVector(DIM), metadata: { type: "plant", diet: "none", pop: 0 } },
			{
				id: "q4",
				vector: randomVector(DIM),
				metadata: { type: "animal", diet: "omnivore", pop: 300 },
			},
			{ id: "q5", vector: randomVector(DIM), metadata: { type: "mineral", pop: 0 } },
		])
		await awaitIndexed()
	})
	...
	test("should include metadata when requested", async () => {
		const results = await index.query({ vector: baseVec, topK: 1, includeMetadata: true })
	...
	test("should not include metadata when not requested", async () => {
		const results = await index.query({ vector: baseVec, topK: 1, includeMetadata: false })
		expect(results[0].metadata).toBeUndefined()
	})

	test("should include vectors when requested", async () => {
		const results = await index.query({ vector: baseVec, topK: 1, includeVectors: true })
