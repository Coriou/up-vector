...
describe("tokenizer", () => {
	...
	test("dotted identifier", () => {
		...
		expect(tokens[0]).toMatchObject({ type: "IDENTIFIER", value: "geo.continent" })
	})
	...
	test("backward indexing", () => {
		...
		expect(tokens[0]).toMatchObject({ type: "IDENTIFIER", value: "items[#-1]" })
	})
...
describe("parser", () => {
	test("simple equality", () => {
		...
		expect(ast).toEqual({ type: "comparison", field: "country", op: "=", value: "Turkey" })
	})

	test("numeric comparison", () => {
		const ast = parse(tokenize("score >= 0.8"))
		expect(ast).toEqual({ type: "comparison", field: "score", op: ">=", value: 0.8 })
	})
	...
	test("GLOB", () => {
		...
		expect(ast).toEqual({ type: "glob", field: "title", pattern: "The *", negated: false })
	})

	test("NOT GLOB", () => {
		const ast = parse(tokenize("title NOT GLOB 'Bad*'"))
		expect(ast).toEqual({ type: "glob", field: "title", pattern: "Bad*", negated: true })
	})
	...
	test("CONTAINS", () => {
		...
		expect(ast).toEqual({ type: "contains", field: "tags", value: "featured", negated: false })
	})

	test("NOT CONTAINS", () => {
		const ast = parse(tokenize("tags NOT CONTAINS 'spam'"))
		expect(ast).toEqual({ type: "contains", field: "tags", value: "spam", negated: true })
	})
	...
	test("HAS NOT FIELD", () => {
		...
		expect(ast).toEqual({ type: "has_field", field: "deleted_at", negated: true })
	})
