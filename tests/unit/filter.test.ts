...
describe("parser", () => {
  ...
  test("complex compound expression", () => {
    const ast = parse(
      tokenize("genre IN ('comedy', 'drama') AND year > 2020 OR rating >= 4.5"),
    );
...
describe("evaluator", () => {
  ...
  // IN / NOT IN
  test("IN matches", () => {
    expect(evaluate(parse(tokenize("color IN ('red', 'blue')")), meta)).toBe(
      true,
    );
    expect(
      evaluate(parse(tokenize("color IN ('green', 'yellow')")), meta),
    ).toBe(false);
  });

  test("NOT IN", () => {
    expect(
      evaluate(parse(tokenize("color NOT IN ('green', 'yellow')")), meta),
    ).toBe(true);
    expect(
      evaluate(parse(tokenize("color NOT IN ('red', 'blue')")), meta),
    ).toBe(false);
  });
  ...
  test("CONTAINS on array", () => {
    expect(evaluate(parse(tokenize("tags CONTAINS 'featured'")), meta)).toBe(
      true,
    );
    expect(evaluate(parse(tokenize("tags CONTAINS 'archived'")), meta)).toBe(
      false,
    );
  });
  ...
  test("NOT CONTAINS on array", () => {
    expect(
      evaluate(parse(tokenize("tags NOT CONTAINS 'archived'")), meta),
    ).toBe(true);
    expect(
      evaluate(parse(tokenize("tags NOT CONTAINS 'featured'")), meta),
    ).toBe(false);
  });
  ...
  test("HAS FIELD", () => {
    ...
    expect(evaluate(parse(tokenize("HAS FIELD nonexistent")), meta)).toBe(
      false,
    );
  });

  test("HAS NOT FIELD", () => {
    expect(evaluate(parse(tokenize("HAS NOT FIELD nonexistent")), meta)).toBe(
      true,
    );
  ...
  test("HAS FIELD with dot notation", () => {
    expect(evaluate(parse(tokenize("HAS FIELD geo.continent")), meta)).toBe(
      true,
    );
    expect(evaluate(parse(tokenize("HAS FIELD geo.missing")), meta)).toBe(
      false,
    );
  });
  ...
  test("dot notation comparison", () => {
    expect(evaluate(parse(tokenize("geo.continent = 'Asia'")), meta)).toBe(
      true,
    );
  ...
  test("deep dot notation", () => {
    expect(evaluate(parse(tokenize("nested.deep.value >= 100")), meta)).toBe(
      true,
    );
    expect(evaluate(parse(tokenize("nested.deep.value < 100")), meta)).toBe(
      false,
  ...
  test("AND both true", () => {
    expect(
      evaluate(parse(tokenize("color = 'red' AND active = true")), meta),
    ).toBe(true);
  });

  test("AND one false", () => {
    expect(
      evaluate(parse(tokenize("color = 'red' AND active = false")), meta),
    ).toBe(false);
  });

  test("OR one true", () => {
    expect(
      evaluate(parse(tokenize("color = 'blue' OR active = true")), meta),
    ).toBe(true);
  });

  test("OR both false", () => {
    expect(evaluate(parse(tokenize("color = 'blue' OR count = 0")), meta)).toBe(
      false,
    );
  });
  ...
  test("population AND continent filter", () => {
    ...
    expect(
      evaluateFilter(
        "population >= 2000000 AND geography.continent = 'Asia'",
        m,
      ),
    ).toBe(false);
  });
  ...
  test("genre IN AND year filter", () => {
    ...
    expect(
      evaluateFilter("genre IN ('comedy', 'drama') AND year > 2020", m),
    ).toBe(true);
    expect(evaluateFilter("genre IN ('action') AND year > 2020", m)).toBe(
      false,
    );
  });
  ...
  test("CONTAINS AND HAS FIELD", () => {
    ...
    expect(
      evaluateFilter("tags CONTAINS 'featured' AND HAS FIELD premium", m),
    ).toBe(true);
  });
