  ...
  switch (node.type) {
    ...
    case "comparison":
      return evalComparison(
        resolveField(metadata, node.field),
        node.op,
        node.value,
      );
    case "glob": {
      ...
      const match =
        typeof val === "string" && globToRegex(node.pattern).test(val);
...
function looseEqual(a: unknown, b: unknown): boolean {
  ...
  if (typeof a === "boolean")
    return a === (b === 1 || b === true || b === "true");
  if (typeof b === "boolean")
    return b === (a === 1 || a === true || a === "true");
  return a === b;
}
...
export function globToRegex(pattern: string): RegExp {
  if (pattern.length > MAX_GLOB_PATTERN_LENGTH) {
    throw new Error(
      `Glob pattern too long (max ${MAX_GLOB_PATTERN_LENGTH} chars)`,
    );
  }
